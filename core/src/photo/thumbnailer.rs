...
impl PhotoThumbnailer {
    ...
    /// Make all the necessary sizes, from XLarge to Small, with one decode
    /// operation.
    async fn thumbnail_all_internal(
        &self,
        path: &FlatpakPathBuf,
        quality: ThumbnailQuality,
    ) -> Result<()> {
        let src_image = self.decode(path).await?;
    ...
    async fn decode(&self, path: &FlatpakPathBuf) -> Result<image::DynamicImage> {
        ...
        Ok(texture_utils::texture_to_rgba(frame.texture())?)
    }
}
